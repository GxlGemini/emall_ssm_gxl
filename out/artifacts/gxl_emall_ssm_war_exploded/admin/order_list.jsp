         ...
         <tr>
         	...
         	<td>
	         	<c:forEach var ="item" items="${order.itemList}">
		         	<a target="_blank" href="../index/detail?id=${item.goods.id}"><p>${item.goods.name}</p></a>
			...
			<td>
				<c:if test="${order.status==2}">
					<a class="btn btn-success" href="orderSend?id=${order.id}&status=${order.status}">发货</a>
				</c:if>
				<c:if test="${order.status==3}">
					<a class="btn btn-warning" href="orderFinish?id=${order.id}&status=${order.status}">完成</a>
				</c:if>
				<a class="btn btn-danger" href="orderDelete?id=${order.id}&status=${order.status}">删除</a>
			</td>
       	</tr>
	</c:forEach>
